...
================================================
Compatible with: py-tgcalls 2.2.x, pyrogram 2.x
...
from utils import get_youtube_info, get_spotify_track, format_duration, get_file_info
...
def make_stream(track: dict) -> MediaStream:
    """Build a MediaStream for py-tgcalls 2.x."""
    if track["type"] == "video":
        return MediaStream(
            track["stream_url"],
        ...
        return MediaStream(
            track["stream_url"],
            audio_parameters=AudioQuality.HIGH,
            video_flags=MediaStream.Flags.IGNORE,   # audio only, ignore missing video
        )
...
def build_controls(chat_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(f"📋 Queue ({q_len})", callback_data="queue"),
...
async def stream_next(chat_id: int):
    """Play next queued track, or leave call if queue is empty."""
    ...
    if not track:
        ...
        return
    try:
        await call_py.play(chat_id, make_stream(track))
...
async def handle_update(_, update):
    # StreamEnded fires when a track finishes naturally
...
async def start_cmd(_, msg: Message):
    await msg.reply_text(
        ...
        "**Commands:**\n"
        "• `/play <song / URL>` — Play audio (YouTube · Spotify · link)\n"
        ...
        "• `/now` — Now playing\n\n"
        "Reply to an audio/video file and use `/play` to stream it."
    )
...
async def _play(msg: Message, video: bool = False):
    ...
    # ── File upload (reply to audio/video) ──
    ...
    if reply and (reply.audio or reply.video or reply.voice or reply.document):
        ...
        try:
            ...
            track = {
                ...
                "requester": msg.from_user.mention if msg.from_user else "User",
            }
    ...
    if not query:
        await msg.reply_text("❗ Provide a song name, YouTube/Spotify URL, or reply to a file.")
        return
    ...
    try:
        ...
        if not track:
            await status.edit_text("❌ Could not find that track. Try a different search.")
            return
        await _enqueue_and_play(msg, chat_id, track, status)
    except Exception as e:
        await status.edit_text(f"❌ Error: `{e}`")
...
async def resolve_track(query: str, video: bool, msg: Message) -> Optional[dict]:
    ...
    if "spotify.com/track" in query:
        if not sp:
            raise RuntimeError("Spotify credentials not configured in .env")
...
async def _enqueue_and_play(msg: Message, chat_id: int, track: dict, status: Message):
    ...
    try:
        await call_py.play(chat_id, make_stream(track))
        queues.pop(chat_id)                     # remove from queue, it's now live
        await db.set_current(chat_id, track)
        ...
        try:
            if thumb:
                await status.delete()
                await msg.reply_photo(thumb, caption=caption,
...
async def pause_cmd(_, msg: Message):
    try:
        await call_py.pause(msg.chat.id)
...
async def resume_cmd(_, msg: Message):
    try:
        await call_py.resume(msg.chat.id)
...
async def loop_cmd(_, msg: Message):
    ...
    await msg.reply_text(f"🔁 Loop **{'ON ✅' if state else 'OFF ❌'}**")
...
async def now_cmd(_, msg: Message):
    ...
    if not current:
        await msg.reply_text("🔇 Nothing playing right now.")
        return
    await msg.reply_text(
        f"🎵 **Now Playing**\n\n"
        f"**{current['title']}**\n"
...
async def cb_handler(_, cq: CallbackQuery):
    ...
    try:
        if data == "pause":
            ...
            await cq.answer("\n".join(lines) or "Queue empty", show_alert=True)
            return
